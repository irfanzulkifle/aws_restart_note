...
## Key Takeaways
- System logs stored in `/var/log/`; severity levels from Emergency (0) to Debug (7)
- Key log files: `syslog`, `secure` (Red Hat), `auth.log` (Ubuntu), `kern.log`, `httpd/`
- Bash challenge: Auto-incrementing file creator using `wc -l` and `$(ls | wc -l)`
- OSI Model: 7 layers — focus on Layer 7 (Application), Layer 4 (Transport), Layer 3 (Network)
