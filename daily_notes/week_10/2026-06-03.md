# Lecture Notes — June 3, 2026
**Cohort 3 | Project CloudIgnite**
**Topics:** SQL Advanced Queries, Amazon RDS
**Duration:** ~3 hours
...
- GROUP BY + HAVING: group rows, then filter groups; `WHERE` filters rows, `HAVING` filters groups
- Set operations: UNION (dedupe), UNION ALL (keep dupes), INTERSECT (common), MINUS (A not in B)
- JOINs: INNER (matching only), LEFT (all left + matching right), RIGHT (all right + matching left), FULL (all from both)
- Window functions: `OVER(PARTITION BY ... ORDER BY ...)` — aggregate without collapsing rows; `RANK()` assigns rank per partition
- Amazon RDS: fully managed relational DB; supports MySQL, PostgreSQL, Aurora, Oracle, SQL Server
