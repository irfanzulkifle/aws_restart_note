...
## Key Takeaways
- Linux file system: everything is a file (processes in `/proc`, hardware in `/dev`)
- File permissions: `rwx` for owner/group/others; `chmod 400` for SSH keys on EC2
- Core commands: `ls`, `cat`, `less`, `head`, `tail`, `cp`, `mv`, `rm`, `mkdir`, `touch`
- Search: `find` locates files by name/attributes; `grep` searches text inside files
- Hard links point to inode (survive deletion); symbolic links point to path (break on deletion)
