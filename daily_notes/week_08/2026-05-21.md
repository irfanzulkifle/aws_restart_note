...
**Topics:** Debugging & Testing, DevOps & Continuous Integration
**Duration:** ~3 hours
...
- Debugging tools: `pdb` (command-line), VS Code debugger (breakpoints, watchers)
- Testing levels: Unit → Integration → System → Acceptance (UAT); use `assert` for validation
- Lab 130: Caesar cipher debugging — Type error (str vs int), logic error (lowercase not in alphabet), wrong parameter to decrypt
