...
## Cohort 3: Project CloudIgnite — June 16, 2026

> **Session length:** ~190 minutes  
> **Topics covered:** REST API requests, responses & HTTP status codes · Amazon API Gateway · AWS Step Functions · Containers vs. Virtual Machines · Docker · AWS Container Services (ECR, ECS, EKS, Fargate) · Lab 177 (S3 → Lambda → SNS) · Lab 179 (Migrate database from EC2 to Amazon RDS)

> [!NOTE]
> Sections marked **🎯 CLF-C02 Relevant** map to learning objectives of the **AWS Certified Cloud Practitioner (CLF-C02)** exam. A consolidated exam-mapping table is at the end.
...
- ⚠️ **Lab 179 (EC2→RDS migration):** several students hit issues (wrong host, wrong security group). **Submit what you have**; the instructor will **re-run this lab Saturday 2:00 PM** (then yesterday's Lambda lab around 4:00 PM).
- 🗓️ **Tomorrow is a holiday** — next class is **Thursday**.
