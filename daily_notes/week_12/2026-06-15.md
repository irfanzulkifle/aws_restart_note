# Lecture Notes — June 15, 2026
**Cohort 3 | Project CloudIgnite**
**Topics:** AWS Lambda, Amazon Route 53 Failover Routing, Lambda + SNS Sales Report, REST API Fundamentals
**Duration:** ~3 hours

---

## Key Takeaways
- **AWS Lambda** is a fully managed, event-driven serverless compute service; you only build & deploy code and monitor the application — AWS handles everything else
- **Lambda billing** is per-invocation + duration (sub-second metering); no charge while idle, in contrast to EC2
- **Lambda has a 15-minute maximum execution time per invocation** (not a daily limit); tasks exceeding this return an error
- **Lambda Layers** package shared libraries/dependencies separately from function code, keeping the handler clean
- **Route 53 failover routing** is an active/passive DNS pattern: primary handles traffic, health checks trigger automatic redirect to secondary on failure
- **S3 → Lambda → SNS** is the canonical event-driven serverless pattern: an S3 upload invokes a Lambda, which publishes to an SNS topic for email/notification
- **Lambda needs an IAM execution role** with permissions (e.g., CloudWatch Logs, VPC access); the role ARN is passed when creating the function
- **ARN confusion is a common exam/lab gotcha:** IAM role ARN ≠ SNS topic ARN ≠ SNS subscription ARN — match the right ARN to the right field
...
- **Saturday 2:00 PM** — instructor will re-run Lab 178 (Lambda + SNS) for anyone who needs it.
- **Tomorrow's lab** uses **S3 instead of the cloned DB** and is ~70–80% similar to Lab 178, **but has no instructions** (a challenge lab). Expect to work independently. Plan: topic first, early break (~7:40), then two labs.
